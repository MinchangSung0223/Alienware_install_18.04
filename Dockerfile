...
RUN /bin/bash -c "cd ~/libraries;git clone https://github.com/IntelRealSense/librealsense.git;cd librealsense; mkdir build; cd build; cmake ..; make -j16; make install;cd ~/workspace/; git clone https://github.com/tjdalsckd/libfranka_gpu_voxel;"
...
RUN /bin/bash -c "cd ~/workspace/; git clone https://github.com/tjdalsckd/ros_trajectory_subscriber.git;"
